...
class DatasetKunstWien extends Dataset {
  ...
  listEntry (entry, dom) {
    ...
    a.innerHTML = '<span class="Bezeichnung">' + escHTML(entry.SEITENNAME) + '</span><span class="Adresse">' + escHTML(this.entryType(entry)) + ' | ' + escHTML(entry.ADRESSE) + '</span>'
    a.href = '#' + this.id + '/' + entry.PAGEID
    td.appendChild(a)
