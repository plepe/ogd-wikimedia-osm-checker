...
function check (options, ob) {
  ...
  if (!ob.data.osm && ob.data.wikidata.length) {
    ob.load('osm', 'nwr["' + options + '"=' + ob.id + '];')
    return ob.load('osm', 'nwr[wikidata="' + ob.data.wikidata[0].id + '"];')
  }

  if (!ob.data.osm) {
    return ob.load('osm', 'nwr["' + options + '"=' + ob.id + '];')
  }
  ...
  if (refBdaResult.length && refWdResult.length && refBdaResult.length !== refWdResult.length) {
    return ob.message('osm', STATUS.ERROR, 'Unterschiedliche Anzahl von Objekten mit <tt>' + options + '=' + ob.id + '</tt> und/oder <tt>wikidata=' + wikidataId + '</tt> in der OpenStreetMap gefunden: ' + ob.data.osm.map(el => '<li>' + osmFormat(el, ob) + '</li>').join('') + '</ul>')
  }

  if (refBdaResult.length) {
    return ob.message('osm', STATUS.SUCCESS, refBdaResult.length + ' Objekt via <tt>' + options + '=' + ob.id + '</tt> gefunden: ' + refBdaResult.map(el => '<li>' + osmFormat(el, ob) + '</li>').join('') + '</ul>')
  }
  ...
  if (refWdResult.length) {
    ob.message('osm', STATUS.ERROR, 'Kein Eintrag mit <tt>' + options + '=' + ob.id + '</tt> in der OpenStreetMap gefunden!')
  ...
  return ob.message('osm', STATUS.ERROR, 'Kein Eintrag mit <tt>' + options + '=' + ob.id + '</tt> ' + (wikidataId ? 'oder <tt>wikidata=' + wikidataId : '') + '</tt> in der OpenStreetMap gefunden!')
}
